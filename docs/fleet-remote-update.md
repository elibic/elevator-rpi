...
מחרוזות מדויקת; `LATEST_VERSION` קבוע בראש `admin-dashboard.js`, פורמט `YYYY.MM.DD`).

הסוכן מדווח `version` לפי **תאריך ה-commit של `HEAD`** (`git log -1 --date=format:%Y.%m.%d`).
היתרון: הערך מתעדכן אוטומטית אחרי כל `git pull`, בלי לתחזק קובץ-גרסה על ה-Pi.

> **על האדמין** לעדכן את `LATEST_VERSION` בדשבורד לתאריך ה-commit של ה-tip ב-`main`
> בכל שחרור. אם תאריך ה-commit של ה-Pi שווה ל-`LATEST_VERSION` → "מעודכן"; אחרת
> "עדכון זמין".

עקיפה אופציונלית: קובץ `VERSION` בשורש הריפו (אם קיים, תוכנו גובר על תאריך ה-commit) —
שימושי אם רוצים לנתק את תווית-הגרסה מתאריך ה-commit.
