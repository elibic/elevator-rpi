...
- **מימוש:** `fsm.check_post_window_exit()` (מנגנון 4 ב-watchdog) - יורה כש: החלון נסגר לפני
  `POST_WINDOW_GRACE_MIN` (15 דק') **וגם** עברו `POST_WINDOW_CYCLE_FACTOR` (2.5) × זמן-המחזור
  הצפוי מאז המחזור התקין האחרון (2.5 סופג מחזור-מדידה שנפל שלם). כיבוי:
...
- **תוצאות מדודות (4 שבתות × 4 מעליות):** B ב-15/8: **23:25 → 20:50** (הבדלה+35 דק'); B ב-1/8:
  +45→+35 דק'; C ב-1/8: +28→+20 דק'; A/D: זהה או 1-3 דק' מהיר יותר. **אפס יציאות לפני ההבדלה
  בכל 16 השחזורים, גם לפני וגם אחרי.** בדיקות: `tests/test_post_window_exit.py`.
