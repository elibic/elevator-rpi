...
- **⚠️ הזמנים חייבים להישאר של ישראל - ו-hebcal נלחם בזה (`_DIASPORA_TZID`):** הלקוח הוא **מלון
  בישראל** שאורחיו בני-חו"ל שומרים יו"ט שני **לפי זמני ישראל**, ולכן צריך **סכימת-חגים** של חו"ל
  עם **מיקום** של הבניין. המשיכה השנייה נשלחה בלי `geonameid` (רק `tzid`) - לוח נכון, **זמנים של
  מקום אחר**. אבל **הוספת `geonameid` לא פותרת:** hebcal **כופה `i=on`** לכל מיקום שהוא מזהה
  כישראלי, והוא מזהה **לפי ה-`tzid`** - גם `geonameid=281184` וגם קואורדינטות-ירושלים גולמיות עם
  `tzid=Asia/Jerusalem` חוזרים עם סוכות ב' כחוה"מ ו-`i=on` מהדהד בכל `link` (אומת מול ה-API החי,
  31/8/26). **הפתרון:** משיכה שנייה ב-`geo=pos` עם **אותן קואורדינטות** (מגיעות מ-`location` של
  התשובה הראשונה ⇒ בלי הגדרה חדשה, נגררות אחרי `GEO_NAME_ID`) ותחת **`tzid` חלופי**
  (`Europe/Athens`). **אף זמן לא זז:** hebcal גוזר הדלקת-נרות/הבדלה מה-lat/lon ורק **מרנדר** אותם
  ב-tzid שנתת, וכל חותמת נושאת את ההיסט שלה שאנחנו תמיד מפרסרים - נמדד בסוכות תשפ"ז: ההבדלה
  בלוח ישראל והדלקת-הנרות של היום השני בלוח חו"ל הן **אותו רגע בדיוק** (`19:07:00+03:00`). אתונה
  נבחרה רק כי היא באותו היסט כמעט תמיד, כדי שהלוגים ייקראו הגיוני. אין `location` בתשובה ⇒
  מדלגים על המשיכה השנייה במקום לשאול מקום אחר. תוקן ב-`hebcal_gate.py`, ב-`schedule_windows.py`,
  וגם ב-web/kiosk (`kiosk-logic.js`, SHARED_UI 1.0.24).
- **`schedule_windows` - שני הלוחות **לא** ממוזגים יותר (חור של שעתיים):** המנוע החזיק רשימת
  starts/ends אחת משני הלוחות, ואז במוצ"ש של יו"ט שני ההבדלה **הישראלית** נכנסה לרשימה, הפכה
  ל-"end הראשון אחרי ה-start הפעיל" וסגרה חלון שלוח חו"ל משאיר פתוח - **26/9 20:37-22:20 לא
  במצב שבת**, באמצע ליל יו"ט. שוחזר על ה-payloads האמיתיים. עכשיו מוחזקות שתי מערכות
  (`_starts/_ends` + `_d_starts/_d_ends`), כל אחת נבדקת בנפרד וה-או ביניהן הוא התשובה - בדיוק
  כמו ה-web (`checkWindows(israel) || checkWindows(diaspora)`). קובץ-state ישן בלי `d_starts`
  נטען כ"ישראל בלבד" והמשיכה הבאה משלימה. **בשער עצמו אין את הבעיה** - הוא עונה מ**איחוד**,
  שיכול רק להרחיב.
...
- **בדיקות:** `tests/test_hebcal_gate.py` (34 חדשות) + `TestRealSukkotPayloads` (payloads אמיתיים מוקלטים) + `TestHardExitAcrossYomTovSheni` ב-
  ...
  `python3 -m pytest tests/` - 143 עוברות.
