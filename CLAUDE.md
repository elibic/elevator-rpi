...
- **⚠️ המשיכה השנייה נושאת את אותו `geonameid` (תיקון גם ב-`schedule_windows.py`):** הלקוח הוא
  **מלון בישראל** שאורחיו בני-חו"ל שומרים יו"ט שני **לפי זמני ישראל**. המשיכה השנייה נשלחה בלי
  `geonameid` (רק `tzid`) - כלומר לוח-החגים של חו"ל אבל **זמני שקיעה של מקום אחר**, כך שהיום השני
  היה נפתח ונסגר בשעה שגויה. עכשיו משתנה **רק `i=off`** (סכימת-החגים); המיקום זהה בשתי המשיכות.
  זה תוקן ב-`hebcal_gate.py` **וגם** ב-`schedule_windows.py`. **בצד ה-web הבאג עדיין קיים**
  (`kiosk-logic.js`: `urlDiaspora` בלי `geonameid`) - נדרש תיקון מקביל + הקפצת `SHARED_UI_VERSION`.
...
- **בדיקות:** `tests/test_hebcal_gate.py` (29 חדשות) + `TestHardExitAcrossYomTovSheni` ב-
  ...
  `python3 -m pytest tests/` - 132 עוברות.
